...
router.post('/', async (req, res) => {
  ...
  if (card.level >= 3) return res.status(400).json({ error: 'Kart max seviyede.' });

  // Enerji ve progress hesapla
  const usable = Math.min(count, user.energy, Math.ceil((100 - card.progress) / 2));
  if (usable < 1) return res.status(400).json({ error: 'Yetersiz enerji veya progress dolu.' });
  ...
  res.json({ progress: card.progress, energy: user.energy, level: card.level, levelUp });
});
